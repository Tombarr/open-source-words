...
READMES = sorted([f for f in listdir_nohidden(READMES_DIR) if isfile(join(READMES_DIR, f))])
...
for README in READMES:
    ...
    with open(read_file_name, "r") as read_file:
        ...
        if ext is 'html':
            out_text = html_to_text(contents)
        elif ext is 'rst':
            out_text = rst_to_text(contents)
        elif ext in MARKDOWN_EXTENSIONS:
            out_text = markdown_to_text(contents)
